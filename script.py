...
def lancer_car(x, y, x1, y1, x2, y2, x3, y3, x4, y4, dx, dy, carlang, largeur, hauteur, couleur):
    ...
    car = Frame2.create_rectangle(x, y, x + carlang, y + carlang, fill=couleur)
...
def deplacement():
    global dx, dy
    if Frame2.coords(raquette)[2] > 780:
        dx = -1 * dx
    if Frame2.coords(raquette)[2] < 50:
        dx = -1 * dx
    if Frame2.coords(raquette)[3] > 200:
        dy = -1 * dy
    if Frame2.coords(raquette)[3] < 30:
        dy = -1 * dy
    # On deplace la balle :
...
#raquette = Frame2.create_rectangle(a0,b0,coleur,z1,fill='yellow')
#raquette1 = Frame2.create_rectangle(x, y, largeur, hauteur, width=30 )
